...
def main(args):
    input_fn_train =  EstimatorOpticFlowInputFunc(dataset_index_filepath='../../data/alov300++/alov300_train.csv',
                                                  ...
                                                  num_epochs=1,
                                                  cache_id='train_shuffled_with_mask_ds_fixed',
                                                  flow_method='fb',
                                                  shuffle=True)
    input_fn_eval = EstimatorOpticFlowInputFunc(dataset_index_filepath='../../data/alov300++/alov300_test.csv',
                                                ...
                                                batch_size=BATCH_SIZE,
                                                num_epochs=1,
                                                cache_id='eval_shuffled_with_mask_ds_fixed',
                                                flow_method='fb',
                                                shuffle=False)

    timestamp = datetime.fromtimestamp(time()).strftime('%m-%d-%H-%M-%S')
    model_dir = '../../models_pairwise2/alov300/%s_s=%s_%s' % (DESC, SPEED_SCALER, timestamp)
    # model_dir =  '../../models_e2e/alov300/s=4_07-01-17-29-07'

    model_fn = MTTracker(mt_params_path='../../params_MT_654.pkl',
